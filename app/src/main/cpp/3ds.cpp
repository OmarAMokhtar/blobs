...
int CLoad3DS::fread(void* dst, int word_size, int words, int dummy)
{
	int bytes = word_size*words;
	memcpy(dst, m_Buffer+m_Index, bytes);
	m_Index += bytes;
    gLoadedBytes += bytes;
...
bool CLoad3DS::Import3DS(t3DModel *pModel, const char *buffer)
{
